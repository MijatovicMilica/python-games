...
class Snake:
    def __init__(self, game):
        ...
        self.rect.center = self.get_rendom_position()
    ...
    def check_food(self):
        if self.rect.center == self.game.food.rect.center:
            self.game.food.rect.center = self.get_rendom_position()
            self.length += 1
    ...
    def draw(self):
        [pg.draw.rect(self.game.screen,"red", segment) for segment in self.segments]
...
class Food:
    def __init__(self, game):
        ...
        self.size = game.TILE_SIZE
        self.rect = pg.rect.Rect([0, 0, game.TILE_SIZE - 2, game.TILE_SIZE - 2])
        self.rect.center = self.game.snake.get_rendom_position()
    def draw(self):
